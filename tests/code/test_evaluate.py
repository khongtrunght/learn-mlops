...
def df():
    data = [
        ...
        {"text": "This is a short text.", "tag": "other"},
...
@pytest.mark.parametrize("f, indices", [(evaluate.nlp_cnn, [0]), (evaluate.short_text, [0, 1])])
def test_slice_functions(df, f, indices):
    assert slice_dataframe(df=df, slicing_function=f).index.to_list() == indices


def test_get_slice_metrics(df):
    ...
    assert metrics["nlp_cnn"]["precision"] == 1.0
    assert metrics["nlp_cnn"]["recall"] == 1.0
    assert metrics["short_text"]["precision"] == 0.5
    assert metrics["short_text"]["recall"] == 0.5
